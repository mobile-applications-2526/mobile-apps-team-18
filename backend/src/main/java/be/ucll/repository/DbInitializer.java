...
public class DbInitializer {
        ...
        public DbInitializer(PasswordEncoder passwordEncoder, UserRepository userRepository,
                        EventRepository eventRepository, TaskRepository taskRepository, DormRepository dormRepository) {
        ...
        public void init() {
                ...
                Task task3 = new Task("Task 3", "who cares", TaskType.DISHES, LocalDate.now().plusDays(3), nathan);

                task1.setAssignedUser(rajo);
                task2.setAssignedUser(sander);
                task3.setAssignedUser(nathan);

                taskRepository.save(task1);
                taskRepository.save(task2);
                taskRepository.save(task3);

                Dorm dorm1 = new Dorm("Blijde-Inkomstraat", "AAA");
